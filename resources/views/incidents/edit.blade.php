...
<div class="max-w-3xl mx-auto">
    <div class="bg-white shadow rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            ...
            <p class="text-gray-600 mt-1">Update the details of this security incident.</p>
        </div>
        
        <form method="POST" action="{{ route('incidents.update', $incident) }}" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="severity" id="severity" required 
                    ...
                    <option value="low" {{ old('severity', $incident->severity) === 'low' ? 'selected' : '' }}>Low - Minor issue with minimal impact</option>
                    <option value="medium" {{ old('severity', $incident->severity) === 'medium' ? 'selected' : '' }}>Medium - Moderate issue with some impact</option>
                    <option value="high" {{ old('severity', $incident->severity) === 'high' ? 'selected' : '' }}>High - Serious issue with significant impact</option>
                    <option value="critical" {{ old('severity', $incident->severity) === 'critical' ? 'selected' : '' }}>Critical - Severe issue requiring immediate attention</option>
                </select>
                @error('severity')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
...
- Potential impact or consequences">{{ old('description', $incident->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
